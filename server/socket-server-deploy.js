...
io.on("connection", (socket) => {
  ...
  socket.on(
    "join-whiteboard",
    async ({ whiteboardId, instanceId, user, canEdit }) => {
      console.log(
        `User ${
          user.name
        } (${instanceId}) joined whiteboard ${whiteboardId} with ${
          canEdit === true ? "edit" : "view"
        } access`
      );
      ...
      whiteboard.forEach((userData, existingInstanceId) => {
        if (userData.user.id === user.id) {
          console.log(
            `Removing existing connection for user ${user.name} (${existingInstanceId})`
          );
      ...
      console.log(
        "Current whiteboard users:",
        Array.from(whiteboard.entries()).map(([id, data]) => ({
          id,
          name: data.user.name,
  ...
  socket.on("shape-update-end", ({ whiteboardId, instanceId, shapes }) => {
    console.log(
      `Shape update ended in whiteboard ${whiteboardId} by ${instanceId}`
    );
  ...
  socket.on(
    "eraser-highlight",
    ({ whiteboardId, instanceId, shapesToErase }) => {
      console.log(
        `Eraser highlight in whiteboard ${whiteboardId} by ${instanceId}, highlighting ${shapesToErase.length} shapes`
      );
  ...
  socket.on("clear-canvas", ({ whiteboardId, instanceId }) => {
    console.log(
      `Canvas cleared in whiteboard ${whiteboardId} by ${instanceId}`
    );
  ...
  socket.on("disconnect", () => {
    ...
    whiteboards.forEach((whiteboard, whiteboardId) => {
      ...
      if (userRemoved && removedUserData) {
        console.log(
          `User ${removedUserData.user.name} left whiteboard ${whiteboardId}`
        );
        ...
        if (whiteboard.size === 0) {
          ...
          console.log(
            `Whiteboard ${whiteboardId} cleaned up - no active users`
          );
        }
      }
    });
